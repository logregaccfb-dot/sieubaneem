...
// ==================== POLLING API GỐC (thay cho WebSocket) ====================
const API_URL = "http://fi8.bot-hosting.net:20692/api/his";
const POLL_INTERVAL = 5000; // 5 giây/lần

// Phiên cuối cùng đã xử lý, để không xử lý trùng khi API trả lại các phiên cũ
let lastProcessedSid = null;
if (resultHistory.length > 0) {
    const lastEntry = resultHistory[resultHistory.length - 1];
    lastProcessedSid = lastEntry.phien !== undefined && lastEntry.phien !== null
        ? parseInt(lastEntry.phien)
        : null;
}

// Xử lý 1 phiên mới (logic giữ nguyên y hệt từ khối WebSocket cũ, chỉ đổi nguồn gọi vào)
function processNewSession(sid, d1, d2, d3) {
...
// Đọc field linh hoạt (API gốc dùng chữ thường: xuc_xac_1, tong, ket_qua...)
function extractSessionFields(item) {
    const sid = item.phien ?? item.Phien ?? item.session ?? null;
    const d1 = item.Xuc_xac_1 ?? item.xuc_xac_1 ?? item.x1 ?? null;
    const d2 = item.Xuc_xac_2 ?? item.xuc_xac_2 ?? item.x2 ?? null;
    const d3 = item.Xuc_xac_3 ?? item.xuc_xac_3 ?? item.x3 ?? null;
    return { sid, d1, d2, d3 };
}

// Gọi API gốc định kỳ, lấy TẤT CẢ phiên mới chưa xử lý (đảo về đúng thứ tự cũ->mới)
async function pollApiGoc() {
    try {
        const response = await fetch(API_URL, { timeout: 10000 });
        const json = await response.json();
        let dataArray = Array.isArray(json.data) ? json.data : (Array.isArray(json) ? json : [json]);

        // API trả về mới nhất ở đầu -> đảo lại thành cũ -> mới để xử lý đúng thứ tự thời gian
        dataArray = dataArray.slice().sort((a, b) => {
            const sa = extractSessionFields(a).sid;
            const sb = extractSessionFields(b).sid;
            return (parseInt(sa) || 0) - (parseInt(sb) || 0);
        });

        for (const item of dataArray) {
            const { sid, d1, d2, d3 } = extractSessionFields(item);
            if (sid === null || d1 === null || d2 === null || d3 === null) continue;

            const sidNum = parseInt(sid);
            if (lastProcessedSid !== null && sidNum <= lastProcessedSid) continue;

            processNewSession(sidNum, d1, d2, d3);
            lastProcessedSid = sidNum;
        }
    } catch (err) {
...
function startPolling() {
    console.log(`[✅] Bắt đầu polling API gốc mỗi ${POLL_INTERVAL / 1000}s: ${API_URL}`);
    pollApiGoc();
    setInterval(pollApiGoc, POLL_INTERVAL);
}
